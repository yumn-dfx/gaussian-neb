...
~~~
!neb=(inter=8,maxiter=30,spring=0.02,step=1,conv=0.0001,init=ldpp,ci)  
~~~
#### 各オプションについて  
- inter=8  始点と終点を結ぶ間の構造の数　デフォルト:10  
- maxiter=20  NEB最適化をする回数の最大値　デフォルト:20  
- spring=0.02 構造間を結ぶバネのばね定数(単位 Hartree/Bohr**2)　デフォルト:0.02  
- conv=0.0001 収束判定条件:各構造にかかるForceのRMSのしきい値　デフォルト:0.0003  
- init=ldpp 初期構造の生成方法  
linear: 始点と終点の座標から直線的に初期構造を生成します．デフォルト  
ldpp: ldpp法による初期構造最適化をします(試験的)．linearで無茶な構造が出力されたときに使うことをおすすめします．
- neb=reg 中間構造にかかる力の計算方法  
org: 最初の論文で報告された方法で力を計算します  
reg: 改良された方法で力を計算します．デフォルト  
ci-eb: CI-EB法で力の計算をします（下記のCIオプションは不要です）  
- opt=fire　最適化の方法  
fire: FIRE法で最適化します．
...
Comments
------------------------
勉強がてら書いてみただけの簡単なスクリプトなので，自由に改変，再利用していただいて構いません（まあ使う人もいないかとは思いますが，一応）．  
